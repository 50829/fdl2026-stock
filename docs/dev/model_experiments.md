# 模型实验记录

本文档记录 2026-05-30 前后在 `src/model_experiments/` 下完成的模型实验、代码变更和阶段性结论。当前所有调参结论都以验证集为准，测试集只作为最终对比参考，不用于调参。

## 代码变更

新增或修改的主要代码：

- `src/model_experiments/run_e0_e1.py`：统一跑 E0/E1 的训练、验证/测试预测、IC/ICIR、Top-N 回测。
- `src/model_experiments/run_ablation.py`：用于 GRU/TCN 的消融实验，支持早停、最佳 checkpoint、特征组选择和不同损失函数。
- `src/data/processed.py`：增加 `cache_in_memory` 路径，避免每个 epoch 反复按日期扫描 parquet；序列样本使用 `sliding_window_view` 批量构造。
- `src/models/sequence/alstm.py`：为 ALSTM/GRU 增加 `use_attention`、`input_layernorm`、`hidden_layernorm` 开关，用于比较注意力和 LayerNorm 变体。
- `src/models/sequence/tcn.py`：新增 TCN 模型，包含因果空洞卷积、残差块和可选时间注意力池化。
- `src/models/__init__.py`、`src/train.py`、`src/predict.py`：接入 TCN，并让训练/预测识别 TCN 为序列模型。

当前基础 `ALSTM`/GRU 结构为：

```text
feature_proj: Linear -> Tanh
GRU
可选时间注意力
线性输出头
```

LayerNorm 变体支持：

```text
input_layernorm=true: LayerNorm(输入特征) -> feature_proj
hidden_layernorm=true: GRU -> LayerNorm(隐状态) -> attention/head
```

## 数据与评估口径

主目标：

```text
label_5d__cs_rank
```

主要指标：

- IC mean：模型分数与 `label_5d__cs_rank` 的逐日 Spearman 相关均值。
- ICIR：IC mean / IC std，用于衡量信号稳定性。
- MSE：辅助参考。
- Top-N 回测：辅助参考，当前策略依赖较强，不能作为唯一模型选择依据。

当前 Top-N 回测策略：

```text
n_hold = 20
k_rotate = 5
step_days = 5
transaction_cost_bps = 5
return_col = label_5d
```

GRU/TCN 评估时已修正 warmup：评估切分内出分，但序列窗口允许使用切分之前的历史特征，避免序列模型少掉切分前 59 个交易日导致指标不可比。

## E0/E1 基线

E0 是 MLP 基线：

```text
112 个特征 -> Linear(112, 256) -> ReLU -> Dropout(0.2) -> Linear(256, 1)
```

E1 是 `src/models/sequence/alstm.py` 中的 ALSTM-GRU：

```text
112 个特征的序列 -> Linear+Tanh -> GRU -> 时间注意力 -> 分数
```

全量测试集最终对比：

| 模型 | 样本数 | MSE | IC | ICIR | 回测收益 | Sharpe | 最大回撤 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| MLP 基线 | 1,280,375 | 0.331642 | 0.082496 | 0.600163 | 2.806080 | 2.428451 | -0.133465 |
| layer1 GRU | 1,019,298 | 0.327920 | 0.085123 | 0.648694 | 0.358659 | 1.248049 | -0.194366 |
| layer1 核心特征 | 1,019,298 | 0.327661 | 0.069877 | 0.492486 | 0.364097 | 1.102746 | -0.167735 |

结论：

- 如果以 IC/ICIR 为主，`layer1 GRU + 112 features` 是当前最好的 GRU 候选。
- MLP 回测收益很高，但 IC/ICIR 低于 `layer1 GRU`，说明当前简化回测策略可能更偏向 MLP 分数分布，不能只看回测收益。

## GRU 结构消融

小样本设置：

```text
train: 2023
valid: 2024
loss: SmoothL1
早停：耐心轮数为 2
```

| 实验 | 特征数 | 隐状态维度 | 层数 | 注意力 | 最佳 epoch | valid 损失 | IC | ICIR | 回测收益 | Sharpe | 最大回撤 |
| --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| base_attn_h128_l2 | 112 | 128 | 2 | yes | 1 | 0.164106 | 0.082912 | 0.399819 | 0.253217 | 0.643299 | -0.413522 |
| hidden64 | 112 | 64 | 2 | yes | 2 | 0.163761 | 0.083066 | 0.384486 | 0.266423 | 0.692241 | -0.395093 |
| layer1 | 112 | 128 | 1 | yes | 1 | 0.162710 | 0.092944 | 0.468959 | 0.480024 | 0.922782 | -0.380244 |
| no_attention | 112 | 128 | 2 | no | 1 | 0.163257 | 0.087882 | 0.431210 | 0.147635 | 0.528733 | -0.419579 |
| core_features | 52 | 128 | 2 | yes | 2 | 0.163933 | 0.084944 | 0.441277 | 0.331567 | 0.782914 | -0.362593 |

结论：

- 1 层 GRU 明显优于 2 层 GRU，说明当前 2 层结构较容易过拟合。
- 去掉注意力后 IC 略有改善但回测变差，注意力对头部排序可能仍有帮助。
- 手工核心特征子集在小样本实验中稳定性尚可，但全量后 IC/ICIR 不如全特征。

## 全量 GRU 候选比较

全量 train/valid：

| 实验 | 特征数 | 层数 | 最佳 epoch | MSE | IC | ICIR | 回测收益 | Sharpe | 最大回撤 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| MLP 基线 | 112 | - | 3 | 0.329789 | 0.097258 | 0.494418 | -0.201582 | -0.047620 | -0.496278 |
| layer1 all features | 112 | 1 | 1 | 0.327678 | 0.095154 | 0.511638 | 0.126012 | 0.488961 | -0.373277 |
| 核心特征 2 层 | 52 | 2 | 1 | 0.327513 | 0.084451 | 0.439894 | -0.031154 | 0.169488 | -0.343132 |
| layer1 核心特征 | 52 | 1 | 1 | 0.327282 | 0.081162 | 0.416585 | 0.332457 | 0.794470 | -0.290193 |

结论：

- 如果以 IC/ICIR 为主，`layer1 all features` 仍是当前 GRU 主候选。
- `layer1 core features` 回撤更低、回测更好，但 IC/ICIR 下降明显，不适合作为主模型。

## 损失函数对照

模型固定为：

```text
layer1 GRU
hidden=128
lookback=60
112 features
attention=true
```

小样本 valid 结果：

| 实验 | 损失函数 | lambda | 最佳 epoch | MSE | IC | ICIR | 回测收益 | Sharpe | 最大回撤 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| loss_layer1_smooth_l1 | SmoothL1 | - | 1 | 0.328197 | 0.092944 | 0.468959 | 0.480024 | 0.922782 | -0.380244 |
| loss_layer1_smooth_l1_corr | SmoothL1 + batch Corr | 0.05 | 1 | 0.329255 | 0.089665 | 0.447459 | 0.614289 | 1.064504 | -0.380975 |
| loss_layer1_mse_corr | MSE + batch Corr | 0.05 | 1 | 0.328137 | 0.089582 | 0.447845 | 0.517058 | 0.957968 | -0.388408 |

结论：

- 以 IC/ICIR 为主，SmoothL1 仍然最好。
- batch CorrLoss 提高了当前回测收益，但降低了 IC/ICIR。
- 主要原因可能是 batch 不是按交易日截面构造，batch correlation 不等价于 daily IC。
- 若要继续优化 IC，应考虑按日期构造 batch，再做 daily CorrLoss。

## 回看窗口对照

模型固定为 `layer1 GRU + SmoothL1 + attention`。

| 实验 | 回看窗口 | 样本数 | 最佳 epoch | MSE | IC | ICIR | 回测收益 | Sharpe | 最大回撤 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| lb20 | 20 | 861,467 | 1 | 0.331110 | 0.083108 | 0.447625 | 0.487723 | 0.919401 | -0.358659 |
| lb30 | 30 | 831,465 | 1 | 0.330489 | 0.083941 | 0.465252 | 0.048113 | 0.369648 | -0.340580 |
| lb60 | 60 | 766,867 | 1 | 0.328197 | 0.092944 | 0.468959 | 0.480024 | 0.922782 | -0.380244 |

结论：

- 从 IC 看，回看窗口为 60 最好。
- 从 ICIR 看，回看窗口为 60 与 30 接近，但 60 仍略高。
- 当前不建议把 20/30 扩到全量。

## LayerNorm 对照

模型固定为：

```text
layer1 GRU
hidden=128
lookback=60
112 features
attention=true
loss=SmoothL1
```

小样本 valid 结果：

| 实验 | 输入 LN | 隐状态 LN | 最佳 epoch | valid 损失 | MSE | IC | ICIR | 回测收益 | Sharpe | 最大回撤 |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| layer1 | no | no | 1 | 0.162710 | 0.328197 | 0.092944 | 0.468959 | 0.480024 | 0.922782 | -0.380244 |
| layer1_input_layernorm | yes | no | 1 | 0.162855 | 0.329607 | 0.087430 | 0.447927 | 0.566943 | 1.034457 | -0.387209 |
| layer1_hidden_layernorm | no | yes | 3 | 0.165485 | 0.332257 | 0.070258 | 0.451512 | 0.453018 | 1.430329 | -0.114050 |
| layer1_input_hidden_layernorm | yes | yes | 4 | 0.166509 | 0.336350 | 0.065219 | 0.426890 | 0.436998 | 0.927140 | -0.354793 |

结论：

- 以 IC/ICIR 为主，三种 LayerNorm 变体都没有超过 `layer1` 基线。
- `input_layernorm` 的回测收益更高，但 IC 和 ICIR 都下降，不适合作为主模型。
- `hidden_layernorm` 和组合 LayerNorm 训练轮数更多，但 IC 明显下降，说明它没有解决当前问题，反而可能压弱截面排序信号。
- 不建议把 LayerNorm 变体扩到全量。

## TCN 实验

新增 TCN 模型：

```text
Input [B, T, F]
-> 因果空洞 Conv1d 残差块
-> 最后一个时间步或注意力池化
-> 线性输出头
```

参数选择参考 TCN 常见设置：

- 因果卷积，避免未来信息。
- dilation 使用 `1,2,4,8`。
- kernel size 使用 3。
- 先用较小通道数，避免 GRU 已观察到的快速过拟合。

小样本结果：

| 实验 | 模型 | 序列长度 | 最佳 epoch | MSE | IC | ICIR | 回测收益 | Sharpe | 最大回撤 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| MLP pilot | MLP | - | - | 0.331217 | 0.090974 | 0.458085 | -0.090438 | 0.124551 | -0.444695 |
| GRU layer1 lb60 | GRU | 60 | 1 | 0.328197 | 0.092944 | 0.468959 | 0.480024 | 0.922782 | -0.380244 |
| GRU layer1 lb30 | GRU | 30 | 1 | 0.330489 | 0.083941 | 0.465252 | 0.048113 | 0.369648 | -0.340580 |
| TCN lb20 c64 | TCN | 20 | 1 | 0.337109 | 0.052631 | 0.262179 | 0.349237 | 0.766966 | -0.398329 |
| TCN lb30 c32 | TCN | 30 | 3 | 0.332273 | 0.074995 | 0.457244 | 0.063735 | 0.391533 | -0.376803 |
| TCN lb30 c32 attn | TCN | 30 | 3 | 0.330684 | 0.077673 | 0.417171 | 0.175830 | 0.560173 | -0.388820 |

结论：

- 当前 TCN 未超过 GRU layer1，也未超过 MLP。
- 轻量 TCN-30 的 ICIR 接近基线，但 IC 明显偏低。
- 注意力池化对 TCN 的 IC 有小幅提升，但 ICIR 下降。
- 暂不建议扩 TCN 到全量。TCN 可以作为报告中的纯深度卷积时序模型尝试，但不是主模型候选。

## 阶段性建议

当前最稳的深度模型候选：

```text
layer1 GRU + 112 个特征 + 回看窗口=60 + attention + SmoothL1 + 早停
```

后续最值得尝试的方向：

1. 做按日期构造 batch 的每日 CorrLoss，使训练目标更贴近 IC。
2. 做 LightGBM 基线和 LightGBM/GRU 排名集成。
3. 用 LightGBM importance 做 Top-K 特征选择，再训练 GRU，替代当前手工核心特征子集。
4. 如果继续做 GRU，只建议围绕当前 `layer1` 做很小范围正则化对照，例如 dropout/weight decay，不建议继续扩大结构搜索。

暂不建议继续扩大 TCN 或 LayerNorm 搜索，除非先重构输入归一化、batch 组织方式或损失函数。
