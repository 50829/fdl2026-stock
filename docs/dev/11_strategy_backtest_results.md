...
6. LightGBM top40 在策略回测上仍然略强于 final 模型。这与此前模型交接文档一致：final 模型的优势主要在 ICIR 和风险解释，简单收益策略上 LightGBM 仍是强基线。
